  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex justify-between items-center">
        <span className="text-[11px] font-medium uppercase tracking-wider text-neutral-500 dark:text-neutral-400">{label}</span>
...
function TagList({ items, variant }) {
  ...
  if (!items?.length) return <span className="text-sm text-neutral-500 italic">None identified</span>;
  return (
    <div className="flex flex-wrap gap-1.5">
      {items.map((item, i) => (
        <span key={i} className={`text-[11px] font-mono px-2.5 py-1 rounded-full ${styles[variant]}`}>{item}</span>
      ))}
    </div>
  );
}
...
function SectionTitle({ icon, title }) {
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      <span className="text-[14px] font-medium uppercase tracking-widest text-neutral-500 dark:text-neutral-400">{title}</span>
    </div>
  );
}
...
function Card({ children }) {
  return (
    <div className="bg-white dark:bg-neutral-900 border border-neutral-800 dark:border-neutral-700 rounded-xl p-6 flex flex-col gap-6">
      {children}
    </div>
  );
}
...
export default function ResultsPage() {
  ...
  const slides = [
    ...
    <Card key="hero">
      <div className="flex items-center justify-center gap-8 flex-wrap">
        ...
        {cats.length > 0 && (
          <div className="flex flex-col gap-3 flex-1 pt-2 min-w-[180px]">
            {cats.map(c => <CategoryBar key={c.label} label={c.label} score={c.score} />)}
          </div>
        )}
        <div className="pt-1 min-w-[200px] max-w-xs">
          <div className="text-[14px] font-bold uppercase tracking-widest text-neutral-500 dark:text-neutral-400 mb-2">Overall Match</div>
          <div className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed" dangerouslySetInnerHTML={{ __html: marked.parse(summary || '') }} />
          {analyzedAt && (
            <div className="text-[11px] font-mono text-neutral-500 dark:text-neutral-500 mt-3">{new Date(analyzedAt).toLocaleString()}</div>
          )}
        </div>
      </div>
    </Card>,
    ...
    categories?.experience && (
      <Card key="experience">
        <div>
          ...
          <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed">{categories.experience.notes}</p>
        </div>
        {categories?.education && (
          <div>
            <SectionTitle icon="◎" title="Education" />
            <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed">{categories.education.notes}</p>
          </div>
        )}
      </Card>
    ),
          ...
          <header className="flex items-center justify-between pr-14 mb-10">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="text-sm text-neutral-500 dark:text-neutral-400 border border-neutral-800 dark:border-neutral-700 px-4 py-2 rounded-lg hover:border-neutral-600 hover:text-black dark:hover:text-white transition-all duration-200"
              >
                ← Slideshow
              </button>
              <button
  ...
  return (
    <div className="h-screen flex flex-col bg-white dark:bg-neutral-950 font-google-sans transition-colors duration-200 overflow-hidden">
      ...
      <div className="flex-1 flex items-center overflow-hidden min-h-0 relative">
        ...
        <button
          ...
          className="absolute left-0 top-0 h-full w-16 z-10 flex items-center justify-center group disabled:opacity-0 disabled:cursor-default transition-opacity duration-200"
          aria-label="Previous"
        >
          <span className="text-2xl text-neutral-300 dark:text-neutral-600 group-hover:text-neutral-500 dark:group-hover:text-neutral-300 transition-colors duration-200 select-none">‹</span>
        </button>
        ...
        <button
          ...
          className="absolute right-0 top-0 h-full w-16 z-10 flex items-center justify-center group transition-opacity duration-200"
          aria-label="Next"
        >
          <span className="text-2xl text-neutral-300 dark:text-neutral-600 group-hover:text-neutral-500 dark:group-hover:text-neutral-300 transition-colors duration-200 select-none">›</span>
        </button>
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-6 py-5 border-t border-neutral-100 dark:border-neutral-800">
        <button
          onClick={goPrev}
          disabled={currentIndex === 0 || isAnimating}
          className="flex items-center gap-2 text-sm text-neutral-500 dark:text-neutral-400 border border-neutral-800 dark:border-neutral-700 px-4 py-2 rounded-lg hover:border-neutral-600 hover:text-black dark:hover:text-white transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed"
        >
          ← Prev
        </button>
